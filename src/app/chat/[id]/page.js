...
const handleAddChat = async () => {
    ...
    try {
      ...
      const getRes = await fetch(`https://chatbot-2vqr.onrender.com/chatbot/get-By/${id}`);
      ...
      const searchRes = await fetch(
        `https://chatbot-2vqr.onrender.com/chatbot/search?message=${encodeURIComponent(morePrompt)}`
      );
  ...
  {chatHistory.map((chat, index) => (
    <div key={index} className="flex flex-col gap-1">
      <div className="text-left bg-gray-200 px-2 py-1 rounded-xl w-fit">
        {chat.userMessage}
      </div>
      <div className="text-right bg-blue-200 px-2 py-1 rounded-xl w-fit self-end">
        {chat.botResponse}
      </div>
    </div>
  ))}
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 18" className="CustomIcon-module__icon___zGR29 CustomIcon-module__icon--standart___0Ap1-"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9 3.75v10.5M3.75 9h10.5"></path></svg>
              ...
              <button onClick={handleAddChat}  className="p-1 mr-2 rounded-full bg-gray-200">
              <div className="w-7 h-6 p-1 ">
